words = {"актёр", "тапёр", "щипцы", "почёт", "аллея", "отчёт", "капля", "дымок"}
wordss = {"дыра", "дом", "рыба", "я", "капуста", "пирог", "рог"}
words = words | wordss  # список слов который может попасться в игре
word = list(words.pop())
print("В слове:", len(word), "букв.")
x = ["_"] * len(word)
lives = 11
while x != word:    # цикл в котором вы либо угодаете слово либо потратите все жизни
    a = input("Введите предпологаемую букву: ")
    if len(a) != 1:
        print("- ты уверен что это буква?")
    chance = 0
